...
const updateWhySchema = z.object({
  title: z.string().trim().min(1).optional(),
  image: z.string().trim().min(1).optional(),
  description: z.string().trim().min(1).optional(),
  extended: z.string().trim().min(1).optional(),
...
export async function PATCH(
  ...
  try {
    ...
    const parseResult = updateWhySchema.safeParse(body);
    ...
    const existingItem = await prisma.whyChooseUsItem.findUnique({
    ...
    const updatedItem = await prisma.$transaction(async (tx) => {
      ...
      return tx.whyChooseUsItem.update({
        where: { id },
        data,
      });
    });
    ...
    console.error("Admin why-choose-us PATCH error:", error);
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.whyChooseUsItem.update({
        where: { id },
    ...
    console.error("Admin why-choose-us DELETE error:", error);
    const response = NextResponse.json({ error: "Internal server error" }, { status: 500 });
    return addSecurityHeaders(response);
  }
}
