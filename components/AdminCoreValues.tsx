...
const emptyForm: Omit<CoreValueItemData, "id" | "number"> & { sortOrder: number | string } = {
  title: "",
  description: "",
  sortOrder: 1,
};
...
export default function AdminCoreValues() {
  const [items, setItems] = useState<CoreValueItemData[]>([]);
  ...
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<keyof CoreValueItemData, string>>>({});
  ...
  const openEditForm = (item: CoreValueItemData) => {
    setForm({
      title: item.title,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const errs: Partial<Record<keyof CoreValueItemData, string>> = {};
    ...
    try {
      const url = editingId ? `/api/admin/core-values/${editingId}` : "/api/admin/core-values";
  ...
  const handleDelete = async (id: string) => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/admin/core-values/${id}`, { method: "DELETE" });
      if (res.ok) {
        await fetchItems();
        setDeleting(null);
      } else {
                ...
                {items.map((s) => (
                  ...
                  >
                    <div className="p-5 flex-grow flex flex-col">
                      <div className="mb-2">
                        <span className="font-serif text-2xl text-terracotta/30 font-light block">
                          {s.number}
                        </span>
                      </div>
                      <div className="mb-3">
